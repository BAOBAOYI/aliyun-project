...
//标签数据
  ...
  var mainlistHeight = $('.main-list').height();
  var mainlistBHeight = $('.main-listB').height();
  var mainlistCHeight = $('.main-listC').height();
  var mainlistDHeight = $('.main-listD-titel').height();
  var mainlistEHeight = $('.main-listD-main').height();
  console.log(mainlistHeight);
  console.log(mainlistBHeight);
  console.log(mainlistCHeight);
  console.log(mainlistDHeight);
  //获取第二部分高度
  ...
  $(window).on('scroll',function (){
    ...
    if(scrollTopValue > topHeight + menuHeight + mainHeight){
      $('.toTop').show();
        $('.nav').css({
          position:'fixed',
          top:0,
    ...
    //导航条下方线条 向右滑动效果
    //判断页面滚动的位置给div.navMove设置位置
    var $navMove = $('#navMove');
    if(scrollTopValue > topHeight + menuHeight + mainHeight){
      $navMove.stop(true,false).animate({
        left:0
      },350,'swing');
      $('#liA').css({
           color:'#00c1dc'
    ...
    if(scrollTopValue > topHeight + menuHeight + mainHeight+mainlistHeight){
        $navMove.stop(true,false).animate({
          left:124
        },350,'swing');
      $('#liB').css({
    ...
    if(scrollTopValue > topHeight + menuHeight + mainHeight+mainlistHeight+mainlistBHeight){
      $navMove.stop(true,false).animate({
        left:248
      },350,'swing');
      $('#liC').css({
    ...
    if(scrollTopValue > topHeight + menuHeight + mainHeight+mainlistHeight+mainlistBHeight+mainlistCHeight){
      $navMove.stop(true,false).animate({
        left:372
      },350,'swing');
      $('#liD').css({
    ...
    if(scrollTopValue > topHeight + menuHeight + mainHeight+mainlistHeight+mainlistBHeight+mainlistCHeight+mainlistDHeight+mainlistEHeight){
      $navMove.stop(true,false).animate({
        left:496
      },350,'swing');
      $('#liE').css({
  ...
  var $divs = $('.main-listB-t').siblings('div');

  for(var i = 0 ; i <$divs.length ; i++){

    $($divs[i]).on('mouseenter',function () {
      $(this).children().stop(true,false).animate({
            top:-120
          },400,'swing').on('mouseleave',function () {
  ...
  $('#chat').on('mouseenter',function () {
    $('.chat-chat').show().on('mouseleave',function () {
      $('.chat-chat').hide();
    })
  })
  ...
  //-----------------------------------------------------------
  //点击滚动到对应的栏目
  var $mainli = $('.main-listL-z li');
  for(var i = 0 ; i <$mainli.length ; i++){
    $($mainli[i]).on('click',function () {
      var ilength = $(this).index();
      $('.main-list').css({
        backgroundImage: ''
      })
      $('.main-list').css({
        backgroundImage: 'url("img/bg'+ilength+'.png")'
      })
    $(this).css({
        backgroundColor: '#00c1dc'
      }).siblings('li').css({
...
// .parents('.main-listL').siblings('.main-listR').children('.main-listR-t'+i).show()
